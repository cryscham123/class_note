...
노트 생성(워크플로 3~4번)은 **리뷰 게이트** 방식으로 codex에 위임한다. codex 는 이 파일만 읽고 자기 레인을 파악한다.
...
**② Claude(메인) — 리뷰어(Gate)**
- 책임: 아래 DoD 자동검증 + 품질 샘플검수. 통과 시 확정(전체 `quarto render`로 리스팅 갱신), 미흡 시 `상태: 수정요청` + 구체 피드백으로 반려.
- 핸드오프: date 애매(멀티데이 녹음)·과목 편집 판단은 오너에게 에스컬레이션.

**③ 오너(사람) — 최종승인**
- 책임: date 확정, 과목 특이 편집 결정, 최종 sign-off. 리뷰어가 막혔을 때만 호출.

### 상태머신 (WORKLOG `상태:` 필드)
`draft → 리뷰대기 → 승인`, 반려 시 `수정요청 → (codex 재작업) → 리뷰대기`.
각 화살표를 당기는 주체: 작성=codex, 리뷰대기/승인/수정요청 전이=리뷰어, date·편집 확정=오너.
